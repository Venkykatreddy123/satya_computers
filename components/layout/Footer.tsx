...
const serviceLinks = [
  ...
  { label: 'Bulk / B2B Orders',          href: '/#b2b' },
];

const customerCareLinks = [
  { label: 'Terms & Conditions', href: '/terms-of-service',  icon: FileText },
  { label: 'Privacy Policy',     href: '/privacy-policy',    icon: ShieldCheck },
  { label: 'Return & Refund',    href: '/refund-policy',     icon: RotateCcw },
  { label: 'Shipping Policy',    href: '/shipping-policy',   icon: Truck },
  { label: 'Track Your Order',   href: '/order-status',      icon: PackageSearch },
];
...
function FooterLinkItem({ href, label, id }: { href: string; label: string; id: string }) {
  return (
    <li>
      <Link
        href={href}
  ...
  return (
    <footer className="relative bg-[#0a0a0a] text-white overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-8">
        ...
        {/* ── Main grid: Brand | Explore | Services | Customer Care | Contact ── */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-10 mb-16">

          {/* ── Brand column (spans 1 on lg) ── */}
          <div className="sm:col-span-2 lg:col-span-1">
            <Link href="/" className="flex items-center gap-3 mb-6 group w-fit">
              ...
              />
              <span className="font-heading text-xl tracking-widest text-white group-hover:text-[var(--color-brand-primary)] transition-colors duration-200">
          ...
          <div>
            ...
            <ul className="space-y-3 list-none p-0 m-0">
              {customerCareLinks.map(({ label, href, icon: Icon }) => (
                <li key={label}>
                  <Link
                    href={href}
                    id={`footer-care-${label.toLowerCase().replace(/\s+/g, '-')}`}
                    className="text-sm text-white/50 hover:text-white transition-all duration-200 inline-flex items-center gap-2.5 group"
                  >
                    <Icon
                      size={13}
                      className="text-[var(--color-brand-primary)]/50 group-hover:text-[var(--color-brand-primary)] shrink-0 transition-colors duration-200"
                    />
            ...
            <ul className="space-y-4 text-sm text-white/60 list-none p-0 m-0">
              ...
              <li className="flex items-start gap-3 border-b border-white/10 pb-5">
                <a
                  ...
                  id="footer-maps-link"
                  className="flex items-start gap-3 group hover:text-white transition-colors duration-200 no-underline text-white/60"
                >
                  <MapPin className="w-5 h-5 text-[var(--color-brand-primary)] shrink-0 mt-0.5 group-hover:scale-110 transition-transform duration-200" />
                  <span className="leading-relaxed font-body group-hover:text-[var(--color-brand-primary)] transition-colors duration-200">
                    {storeInfo.address}
                    <span className="block text-[10px] text-[var(--color-brand-primary)]/60 group-hover:text-[var(--color-brand-primary)] mt-0.5 font-heading tracking-widest uppercase">
                      Get Directions →
                    </span>
                  </span>
                </a>
              ...
              <li className="pt-1">
                ...
                >
                  ...
                  <span className="text-white text-sm font-body tracking-wide border-b border-transparent group-hover:border-[var(--color-brand-primary)] transition-colors">
                    {storeInfo.email}
                  </span>
                </a>
              </li>
